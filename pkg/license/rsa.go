...
func NewGoRSAOrigin(pubKeyFilename, priKeyFilename string) (*GoRSA, error) {
...
func NewGoRSA(pubKeyFilename, priKeyFilename string) (*GoRSA, error) {
	...
	privateKey, err := ioutil.ReadFile(priKeyFilename)
	if err == nil {
		block, _ := pem.Decode(privateKey)
		if block == nil {
			return nil, errors.New("private key error")
		...
		goRSA.PrivateKey = pri
	}
	return goRSA, nil
}

// NewGoRSA 初始化 GoRSA对象
func NewGoRSAPub(pubKey []byte) (*GoRSA, error) {
	goRSA := &GoRSA{}
